...
async def load(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            if not cap.isOpened():
                raise HTTPException(
                    500, "Could not open video file to determine duration."
                )
            ...
            if fps > 0 and frame_count > 0:
                ...
                raise HTTPException(
                    500,
                    "Could not read video properties (FPS, Frame Count) to determine duration.",
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Error determining video duration: {str(e)}"
            )
        finally:
            if cap:
                cap.release()
        ...
        if video_duration_seconds <= 0:
            raise HTTPException(
                400,
                "Video duration must be positive. Calculated duration was zero or negative.",
            )
        ...
        total_estimated_time_seconds = (
            video_duration_seconds / base_video_duration_seconds
        ) * base_processing_time_seconds
...
async def analyze(body: LoadBody):
    ...
    if not os.path.exists(body.file_path):
        raise HTTPException(
            404,
            "Original uploaded file for analysis not found at specified path: "
            + body.file_path,
        )
    ...
    try:
        ...
        )  # Up to aaa-nyc-llama-hackathon
        frontend_target_dir = os.path.join(
            project_root, "aaa_client", "public", "processed_videos"
        )
        ...
        if "video_segments" not in analysis_results:
            return {
                "status": "error",
                "message": "Invalid JSON structure: missing 'video_segments' key",
            }
        ...
        for video_segment in analysis_results["video_segments"]:
            ...
            if not os.path.exists(source_video_segment_path):
                print(
                    f"Warning: Source video segment not found: {source_video_segment_path}. Skipping this segment."
                )
                # Optionally include this info in the response or error out
                continue

            segment_filename = os.path.basename(source_video_segment_path)
            destination_public_path = os.path.join(
                frontend_target_dir, segment_filename
            )

            try:
                shutil.copy2(source_video_segment_path, destination_public_path)
                print(
                    f"Copied: {source_video_segment_path} TO {destination_public_path}"
                )
        ...
        if not output_arr:
            return {
                "status": "warning",
                "message": "No feedback items extracted from analysis results",
                "data": [],
            }
