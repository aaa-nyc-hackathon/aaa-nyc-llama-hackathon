...
def track_player_points(
    ...
    tracking_points = []
    track_history = [
        [] for _ in range(len(pixel_points))
    ]  # Store history of tracked points
    ...
    for point_idx, (x, y) in enumerate(pixel_points):
        ...
        p = cv2.goodFeaturesToTrack(
            roi, maxCorners=1, qualityLevel=0.3, minDistance=7, blockSize=7
        )
    ...
    while True:
        ...
        for idx, p0 in enumerate(tracking_points):
            ...
            p1, status, _ = cv2.calcOpticalFlowPyrLK(
                gray_old, gray_new, p0, None, **lk_params
            )
            ...
            if p1 is None or len(p1[status == 1]) == 0:
                ...
                if new_p is not None and len(new_p) > 0:
                    ...
                    # Draw the new point
                    cv2.circle(
                        display_frame, (int(pt_x), int(pt_y)), 7, (0, 0, 255), -1
                    )
                else:
                    new_tracking_points.append(None)
            else:
                ...
                if len(good_new) > 0:
                    ...
                    for i in range(1, len(track_history[idx])):
                        if (
                            track_history[idx][i - 1] is not None
                            and track_history[idx][i] is not None
                        ):
                            cv2.line(
                                display_frame,
                                track_history[idx][i - 1],
                                track_history[idx][i],
                                (0, 255, 0),
                                2,
                            )
...
if __name__ == "__main__":
    ...
    json_input = {
        ...
        "image_data": [
            {
                "jerseyNumber": 0,
                "coordinates": {"x_coordinate": 0.164, "y_coordinate": 0.469},
                "team": "Clemson",
            },
            {
                "jerseyNumber": 2,
                "coordinates": {"x_coordinate": 0.34, "y_coordinate": 0.503},
                "team": "Alabama",
            },
            {
                "jerseyNumber": 12,
                "coordinates": {"x_coordinate": 0.475, "y_coordinate": 0.499},
                "team": "Alabama",
            },
            {
                "jerseyNumber": 3,
                "coordinates": {"x_coordinate": 0.5, "y_coordinate": 0.378},
                "team": "Alabama",
            },
            {
                "jerseyNumber": 5,
                "coordinates": {"x_coordinate": 0.595, "y_coordinate": 0.446},
                "team": "Alabama",
            },
            {
                "jerseyNumber": 10,
                "coordinates": {"x_coordinate": 0.794, "y_coordinate": 0.633},
                "team": "Clemson",
            },
            {
                "jerseyNumber": 24,
                "coordinates": {"x_coordinate": 0.826, "y_coordinate": 0.614},
                "team": "Clemson",
            },
        ],
    }
    ...
    try:
        ...
        track_player_points(
            video_path=video_file, points=list_of_points, display_only=True
        )
