    ...
    for i, clip_name in enumerate(clips):
        ...
        if i == 0:
            ...
            if output_dir != input_dir:
                ...
                try:
                    ...
                    subprocess.run(ffmpeg_copy_cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
                ...
                subprocess.run(ffmpeg_cmd_retry, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def split_camera_moves(input_video_path, output_dir="video_segments", threshold=1.5):
    ...
    try:
        scene_list = detect(input_video_path, AdaptiveDetector(threshold), show_progress=True)
        split_video_ffmpeg(
            input_video_path,
            scene_list,
        ...
        trim_clips_except_first(input_dir=temp_output_dir, output_dir=output_dir, trim_percent=0.08)
    except Exception as e:
    ...
    #list of video paths
    video_paths = [os.path.join(output_dir, f) for f in os.listdir(output_dir) if f.endswith(('.mp4'))]
    return video_paths
